...
void GenSeed(unsigned char *seed){
	kryzhovnik_randombytes(seed, SEEDBYTES);
...
void GenSecret_s(polyvecl *a) 
{
	...
	if(ncoeffs < PQS_n){
			fprintf(stderr, "ERROR :: Too short buffer in GenSecret_s(), restarting...");
			GenSecret_s(a);
	}
}
...
void GenSecret_y(polyvecl *a, unsigned char *seed) 
{
	...
	if(ncoeffs < PQS_n){
			fprintf(stderr, "ERROR :: Too short buffer in GenSecret_y(), restarting...");
		GenSeed(seed);
		GenSecret_y(a, seed);
	}
}
...
void matrixpoly(const polymatkl *mat, const polyvecl *a, polyveck *res){
	int i, j, k;
	uint64_t int_acc;
	poly acc;
...
void PQS_keygen(vk_t *vk_ptr, sk_t *sk_ptr){
	#define vk (*vk_ptr)
	#define sk (*sk_ptr)
	int i,j;
	...
	#ifdef DEBUG_MODE
	/* debug output disabled in C-only build */
	#endif
...
void PQS_sign(signat_t *sig_ptr, const unsigned char *m, uint32_t mlen,
			  const sk_t *sk_ptr, const vk_t *vk_ptr){
	#define sig (*sig_ptr)
	#define sk (*sk_ptr)
	#define vk (*vk_ptr)
	int i, j;
...
bool PQS_verify(const signat_t *sig_ptr, const unsigned char *m, uint32_t mlen,
				const vk_t *vk_ptr){
	#define sig (*sig_ptr)
	#define vk (*vk_ptr)
	int i,j;
